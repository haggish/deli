          ...
          <div class="card flags-card">
            ...
            <div class="flags">
              @if (stop()!.packageFlags.includes('FRAGILE')) {
                <ion-chip class="flag-chip fragile">
                  <ion-icon name="warning-outline" /><ion-label>Fragile</ion-label>
                </ion-chip>
              }
              @if (stop()!.packageFlags.includes('REQUIRES_SIGNATURE')) {
                <ion-chip class="flag-chip signature">
                  <ion-icon name="ribbon-outline" /><ion-label>Signature required</ion-label>
                </ion-chip>
              }
              @if (stop()!.packageFlags.includes('CASH_ON_DELIVERY')) {
                <ion-chip class="flag-chip cod">
                  <ion-icon name="cash-outline" /><ion-label>Cash on delivery</ion-label>
                </ion-chip>
              }
            </div>
          </div>
        ...
        <div class="actions">
          ...
          @if (stop()!.status === 'PENDING') {
            <ion-button
              expand="block"
              class="start-btn"
              (click)="startStop(stop()!.id)"
              [disabled]="acting()"
            >
              @if (acting()) { <ion-spinner name="crescent" /> }
              @else { I've arrived }
            </ion-button>
          }

          @if (stop()!.status === 'IN_PROGRESS') {
            <ion-button expand="block" class="confirm-btn" (click)="goToConfirm(stop()!.id)">
              Confirm delivery
            </ion-button>
          }
        </div>
...
export class StopDetailPage implements OnInit {
  ...
  mapMarkers = (): MapMarker[] => {
    ...
    return [{
      ...
      color: s.status === 'IN_PROGRESS' ? 'green' : 'yellow',
      popup: `${s.deliveryAddress.street} ${s.deliveryAddress.houseNumber}`,
    }];
  };
  ...
  goToConfirm(stopId: string): void {
    this.router.navigate(['/courier/stop', stopId, 'confirm']);
  }
  ...
  openMaps(): void {
    ...
    if (!s) return;
    window.open(
      `https://maps.google.com/?q=${encodeURIComponent(s.deliveryAddress.formatted)}&ll=${s.latitude},${s.longitude}`,
      '_blank',
    );
  }
  ...
  async copyBuzzer(code: string): Promise<void> {
    ...
    const t = await this.toast.create({
      message: 'Buzzer code copied', duration: 1500, position: 'bottom', color: 'dark',
    });
    await t.present();
  }
}
